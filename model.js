...
(function (root) {
  ...
  function buildExpense({ id, date, items, catPath, amount, credits, store, branch, memo }) { const debits = items ? items.map(it => ({ ref: 'cat:' + it.catPath, amount: it.amount })) : [{ ref: 'cat:' + catPath, amount }]; const cr = credits.map(c => ({ ref: 'acc:' + c.accId, amount: c.amount, qty: c.qty })); return buildMulti({ id, date, debits, credits: cr, store, branch, memo, kind: 'expense' }); }
